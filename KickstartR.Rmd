...
The variable this project set out to predict is the 'binary_state' - whether the project was successful in raising its crowdfunding, or if it failed.
```{r cache=TRUE, message=FALSE}
...
In light of this an attempt was undertaken to train a random forest on the smallest reduced training data set. On this occasion the Rborist algorithm was used in anticipation that it would be faster to train than the randomForest library.
```{r eval=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
...
It may also be the case, that forecasting other variables such as the number of backers or whether the project will be picked as a Staff Pick, as preliminaries to forecasting success itself, may yield even stronger results. Finally, it is noted that even a week after a project's launch much more would be known about the project's popularity, so it may be that a model built to give forecasts throughout the lifecycle of a project could deliver much more accurate forecasts even shortly after a project has been launched. All of these possible approaches would seem to be fruitful avenues of future research.
